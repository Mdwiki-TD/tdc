...
function delete_user_page($id)
{
    ...
    $findIt_1 = fetch_query("SELECT 1 FROM pages_users       WHERE id = ? LIMIT 1", [$id]);
    $findIt_2 = fetch_query("SELECT 1 FROM pages_users_to_main WHERE id = ? LIMIT 1", [$id]);

    // $deleteDone = (empty($findIt)) ? true : false;
